...
class MyReservations {
  ...
  timefunction(el, timeInMinute) {
    ...
    let second = `${Math.trunc(timeInSecond % 60)}`.padStart(2, "0");
    let minute = `${Math.trunc(timeInSecond / 60) % 60}`.padStart(2, "0");
    el.textContent = `${hour}:${minute}:${second}`;
    setInterval(function () {
      timeInSecond--;
      second = `${Math.trunc(timeInSecond % 60)}`.padStart(2, "0");
  ...
  async cancel(id) {
    ...
    const request = await fetch("http://localhost:3000/client/myReservations", {
      method: "DELETE",
      mode: "cors",
      headers: {
